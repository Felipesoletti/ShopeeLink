// ═══════════════════════════════════════════
// ShopeeLinks — app.js (Corrigido)
// ═══════════════════════════════════════════

const PROXY_URL = 'https://shopee-api-proxy.felipeoliveira2.workers.dev';

// Helper: sempre lê credenciais do localStorage
    ...
    nodes.forEach(function(loja) {
      ...
      html +=
        '<div class="cupom-card">' +
          ...
          '<div style="display:flex;align-items:center;justify-content:space-between;gap:6px;flex-wrap:wrap">' +
            (tipo ? '<span style="font-size:10px;background:rgba(255,102,51,0.15);color:var(--orange);padding:3px 8px;border-radius:20px;font-weight:700">' + tipo + '</span>' : '<span></span>') +
            (pct ? '<span class="cupom-code">' + pct + '</span>' : '') +
          '</div>' +
          '<div class="cupom-desc">' + (loja.shopName || 'Loja Parceira') + '</div>' +
          '<div style="display:flex;justify-content:space-between;align-items:center;margin-top:4px">' +
            (stars ? '<div style="font-size:11px;color:var(--orange)">\u2b50 ' + stars + '</div>' : '<div></div>') +
            (fim ? '<div class="cupom-validade">At\u00e9 ' + fim + '</div>' : '') +
          '</div>' +
          '<div style="display:flex;gap:6px;margin-top:10px">' +
            '<a href="' + link + '" target="_blank" class="product-btn product-btn-main" style="flex:1;padding:8px;text-align:center;text-decoration:none;font-size:11px;font-weight:700;border-radius:8px">Ir para loja &#8594;</a>' +
            '<button class="copy-btn" onclick="copiarTexto(this)" data-link="' + link + '" style="font-size:11px">Copiar link</button>' +
          '</div>' +
        '</div>';
    });
...
function copiarTexto(btn) {
  var txt = btn.dataset.link || btn.dataset.code;
  if (!txt) return;
...
async function callAPI(appId, secret, query) {
  ...
  // Assinatura correta da Shopee: appId + timestamp + body + secret
...
async function buscarOfertas() {
  ...
  try {
    ...
    while (allNodes.length < limit) {
      ...
      const query = `{ productOfferV2(keyword: "${keyword}", sortType: ${sortType}, page: ${page}, limit: ${thisLimit}) { nodes { itemId productName imageUrl priceMin priceMax priceDiscountRate commissionRate offerLink shopName } } }`;
      const data = await callAPI(appId, secret, query);
      const nodes = data.data.productOfferV2.nodes || [];
      if (!nodes.length) break;
      allNodes = allNodes.concat(nodes);
      if (nodes.length < thisLimit) break;
      page++;
    }

    const filtered = filtroMall ? allNodes.filter(p => p.shopName && (p.shopName.toLowerCase().includes('oficial') || p.shopName.toLowerCase().includes('mall'))) : allNodes;

    if (!filtered.length) {
      container.innerHTML = `<div class="empty-state"><div class="icon">&#128269;</div><p>Nenhum produto encontrado para "${keyword}".</p></div>`;
      return;
    }

    container.innerHTML = `
      <div style="margin-bottom:12px;font-size:13px;color:var(--muted)">Exibindo ${filtered.length} produtos encontrados</div>
